...
class ScoreRepositoryImplTest {
    ...
    // 테스트 시나리오
    // - 단언(Assertion) 기법
    // 테스트는 void, default 로 만들어야한다.
    @Test
    @DisplayName("저장소에서 findAll을 호출하면 그 리스트에는 성적정보가 3개 있어야 한다.")
    void findAllTest() {
        // given-when-then 패턴
...
//        Assertions.assertTrue(scoreList.size()==3);
    ...
    @DisplayName("저장소에서 findByStuNum 을 호출하여 학번이 2인 학생을 조회하면" +
            "그학생의 국어점수가 33점이고 이름이 춘식이어야만 한다. ")
        // 테스트는 void, default 로 만들어야한다.
    void findOneTest() {
        //given
        int stuNum = 2;
    ...
    void saveTest(){
        ...
        //then
        assertEquals(7,scoreList.size());
        assertTrue(flag);
        assertEquals(7,scoreList.get(scoreList.size()-1).getStuNum());
    }
